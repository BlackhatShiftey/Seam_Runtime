...
Last updated: 2026-05-19 (HISTORY#210 — CI hardening batch: SEAM verify gates, expanded pytest scope, MCP stdio smoke test)
...
- Latest continuity handoff is `HISTORY#210` — CI hardening batch via the synchronous batch sync-relay DeepSeek loop: CI1 added four SEAM verify gates (integrity, continuity, routing, streams) to .github/workflows/ci.yml between Run tests and Run benchmark suite on both matrix legs; CI2 expanded the CI pytest invocation to also cover tools/streams/ and tests/ so the audit suite runs in CI; CI3 added tests/audit/test_mcp_stdio_smoke.py — a subprocess-based MCP stdio JSON-RPC handshake smoke test for the seam_runtime.mcp_protocol entrypoint. Closes audit findings H2, H3a, H3b. `HISTORY#209` is the second leg of the prior WebUI batch (H1 streams fsync, H5 .cursor/ gitignore, M8 TestCountFact rename, PROJECT_STATUS ghost-path removal). `HISTORY#208` is the WebUI batch first leg (W1 /tree, W2 /benchmark, W3 /sys-metrics, W4 record_kinds symbol contract, SOP + prompt + ledger authoring, dashboard consumer update). `HISTORY#207` is the prior Codex review of the LX1 type-preservation tests; `HISTORY#206` is the DeepSeek parallel audit pass; `HISTORY#205` is the SOP document; `HISTORY#204` is the prior audit remediation pass.
