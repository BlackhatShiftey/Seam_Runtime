...
- Latest continuity handoff is `HISTORY#170`. **Track H1 Context Streams substrate is implemented and functional.** `tools/streams/` provides generic stream tooling; root `HISTORY.md` + `HISTORY_INDEX.md` remain canonical with byte-equivalent derived mirrors under `.seam/streams/history/`; new `roadmap` and `experience` streams live under `.seam/streams/`; `.seam/cross_index.md` is the derived global temporal join; `.seam/streams/roadmap/state.md` is the compact agent-facing status view derived from `ROADMAP.md` marker blocks. `seam doctor` reports a `streams` field; the canonical pre-commit hook and the Claude preflight both run `verify_streams` as a fourth gate. AGENTS.md now has a Context Loop section formalizing bounded session-start reads (state.md instead of full ROADMAP.md, cross_index hot zone for cross-stream timeline). 8 streams unit tests pass; full verify chain (`verify_integrity`/`verify_routing`/`verify_continuity`/`verify_streams`) is green. Path canonicality flip for `history` remains deferred to a separate later HISTORY entry per CONTEXT_STREAMS.md §9. Snapshot JSON files are local derived artifacts, so a fresh Linux clone should regenerate the resume snapshot with the command in `docs/setup.md` before running continuity verification.
