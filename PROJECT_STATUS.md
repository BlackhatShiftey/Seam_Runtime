...
Last updated: 2026-05-25 (HISTORY#245 - GitHub PR CI gates)
...
- Latest continuity handoff is `HISTORY#245` - GitHub PR management now has hard repo gates for no-paid LoCoMo BIL-2 quickstart sealing/verification, real Chroma smoke coverage, diff hygiene, and a non-printing secret/session URL scan. `.github/workflows/ci.yml` adds `repo-hygiene`, `chroma-real-smoke`, and `locomo-quickstart-bil2` jobs; `.github/pull_request_template.md` keeps the history/streams, BIL-2 quickstart, no-paid-calls, and secrets checklist visible in review; `tools/ci/chroma_real_smoke.py` is the real `chromadb` smoke helper. `HISTORY#244` remains the latest H2 retrieval_event substrate code slice. Step 4 final paid measurement remains operator-gated.
