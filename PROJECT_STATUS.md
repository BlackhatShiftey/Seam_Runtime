...
Last updated: 2026-05-24 (HISTORY#241 - LoCoMo --keep-db persistent ingest cache)
...
- Latest continuity handoff is `HISTORY#241` - persistent ingest cache for the LoCoMo benchmark inner loop. `SeamLocomoAdapter` accepts `keep_db: bool = False`; with `--keep-db` and `--db-path`, repeat `reset()` keeps the SQLite file on disk and skips re-ingest for cached scopes while still updating the temporal anchor. `HISTORY#240` landed four no-paid retrieval fixes (separated `search_top_k=20` from the 2000-char context budget, preserved ranking order through evidence assembly, per-scope `SeamRuntime` cache, module-level default sentence-transformer cache) and moved the 100-case no-paid slice from 0.358600 to 0.494742 against `/home/terrabyte/seam_benchmarks/track_m/locomo/locomo10.json`. `HISTORY#238` is the paid Step 0b result of record: context_recall_mean=0.2885920093, judge_score_mean=0.1945525292 (46 correct, 508 partial, 988 incorrect, 1245 unknown predictions out of 1542 cases) with BIL-2 result hash 17f4afa8904cfe2d50fb10cc4433ccf651e9530414a5a357e0b9377840803d0c. `HISTORY#239` closed out the snapshot/index rebuild and continuity verify chain after #237 and #238. Cold/warm 100-case timing under `--keep-db --db-path /tmp/seam-track-m/iter_db`: 70.22s cold and 31.78s warm at identical 0.494742 (2.2x speedup); 10-case warm completes in ~9s. Step 4 final paid measurement remains operator-gated.
