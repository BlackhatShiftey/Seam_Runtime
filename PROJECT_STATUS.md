...
Last updated: 2026-05-25 (HISTORY#249 - AGENTS GitHub PR workflow protocol)
...
- Latest continuity handoff is `HISTORY#249` - repo hygiene continued on branch `codex/repo-hygiene-ruleset-record` and draft PR #32. `HISTORY#246` records the GitHub main ruleset change: `Protect main (PR + hygiene gates)` applies to `refs/heads/main` with no bypass actors, blocks deletion/non-fast-forward updates, requires PRs, and requires `repo-hygiene`, `chroma-real-smoke`, and `locomo-quickstart-bil2`. `HISTORY#248` completed the advisory repository-maintenance workflow for open/stale PR and stale-branch reporting. `HISTORY#249` updates `AGENTS.md` so all agents see the PR workflow directly: no direct `main` pushes, isolate unrelated dirty files, keep PRs moving toward merged/closed/active/blocked states, distinguish required checks from advisory matrix failures, and use the maintenance report for stale PR/branch cleanup. PR #32 required checks pass; the broader `test-and-benchmark` matrix remains advisory and still fails on pre-existing CI dependency/platform issues. PR #31 is still a draft/conflicting stale-work candidate, and its generated assistant session-link trailer was redacted from the PR body during HISTORY#248. `benchmarks/external/locomo/adapters/seam.py` and `tests/audit/test_locomo_adapter_retrieval_event_writer.py` remain dirty with unrelated H2 retrieval-event writer-hook work and were not staged for repo hygiene. Step 4 final paid measurement remains operator-gated.
