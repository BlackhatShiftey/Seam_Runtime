...
- Latest continuity handoff is `HISTORY#169`. It installs the canonical cross-agent commit gate at `tools/git-hooks/pre-commit` + `tools/git-hooks/install.sh` (symlink with copy fallback for exFAT/FAT32) and adds a `commit_gate` field to `seam doctor` that reports installed/drift/missing state. Every `git commit` is now gated by `verify_integrity` + `verify_routing` + `verify_continuity` regardless of which agent or human invoked it. The Claude Code hooks from `HISTORY#167`/`HISTORY#168` remain as a defense-in-depth early-warning layer wired through per-machine `.claude/settings.json`. Track H Context Streams Protocol design is locked in `docs/roadmap/CONTEXT_STREAMS.md`; resume implementation by reading that doc end-to-end. Snapshot JSON files are local derived artifacts, so a fresh Linux clone should regenerate the resume snapshot with the command in `docs/setup.md` before running continuity verification.
