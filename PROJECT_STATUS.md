...
- Latest continuity handoff is `HISTORY#168`. It corrects the scope of the Claude Code commit gate install from `HISTORY#167`: only the shared scripts `tools/claude/preflight_protocol.sh` and `tools/claude/session_start_brief.sh` are tracked; each operator wires them into their own per-machine `.claude/settings.json`, which stays gitignored and is additionally blocked by the local `.git/hooks/pre-commit` policy. `HISTORY#166` is the protocol catch-up for the Track H design commit `4cde6e5`. Track H Context Streams Protocol design is locked in `docs/roadmap/CONTEXT_STREAMS.md`; resume implementation by reading that doc end-to-end. Snapshot JSON files are local derived artifacts, so a fresh Linux clone should regenerate the resume snapshot with the command in `docs/setup.md` before running continuity verification.
