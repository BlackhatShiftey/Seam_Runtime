...
Last updated: 2026-05-22 (HISTORY#234 - Track M P4 Step 0a baseline + openai answerer bug fix)
...
- Latest continuity handoff is `HISTORY#234` - Track M P4 Step 0a baseline measurement is landed on a 10-case quickstart, alongside a real bug fix in `benchmarks/external/locomo/adapters/seam.py` `_openai_short_answer` (unconditional `temperature=0` + unscaled `max_completion_tokens` blocked every gpt-5/o-series reasoning model as the LoCoMo answerer). Both quickstart smokes ran via the `--judge-batch` path against the real OpenAI Batch API (first live execution of `OpenAIJudge.score_batch` from HISTORY#232; 10/10 verdicts, 0 batch errors per run). The A/B picked `gpt-5-mini` as the baseline-of-record answerer at recall=0.9633, EM=0.7000, F1=0.9049, judge_score=0.9500 (9c/1p/0i; all three prior `gpt-4o-mini` abstentions resolved correctly), sealed BIL-2 at result hash `f89c848f373039715d039cd4ba323afb9579aa9520288f855f34672868c10da7`. The `gpt-4o-mini` A/B-comparison number (judge_score=0.7000, 3 abstentions) is preserved in HISTORY#234 for the record, not as the baseline; that bundle is also BIL-2-sealed at result hash `50a2d2b52c52ef8b2be51a151523080bdcebd66c6c1959d36a3ac900f3fb4c64`. Step 0b (full 1,542-case LoCoMo with `--answerer-model gpt-5-mini --judge-model gpt-5-nano --judge-batch`) remains operator-gated, estimated <$1 / ~2-3 hours wall time. Phase B answerer batching from HISTORY#232 is still deferred pending Step 0b cost data. `HISTORY#233` remains the no-paid-API temporal-distance scoring slice. Unrelated local edits in `tests/audit/test_bench_stub_seal_gate.py` and `tests/audit/test_benchmark_endpoint_safety.py` predate this session and were not staged in HISTORY#234; they should be reviewed separately. The stale `HANDOFF_BATCH_API.md` was removed by this entry.
