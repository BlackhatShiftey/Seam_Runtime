...
Last updated: 2026-05-21 (HISTORY#220 — Track M P0 standard benchmark readiness merged; next-run real dataset SOP added)
...
- Latest continuity handoff is `HISTORY#220` — Codex review/repair of DeepSeek's Track M P0 branch. DeepSeek's work was uncommitted and left three gaps: LoCoMo quickstart recall was still 0.0, dataset dry-run subcommands were not routed through `seam bench external`, and the publication gate did not actually require a BIL-2 verification report. Codex added regressions and fixed all three. The quickstart smoke `.venv/bin/python -m seam bench external --quickstart locomo --adapter seam --judge stub --format json` now reports `context_recall_mean=0.9633333333333333` (stub smoke-only). The full active suite `.venv/bin/python -m pytest test_seam_all/ tools/history/test_history_tools.py tools/streams/ tests/ -q` passed 503 tests, skipped 6, and passed 3 subtests. Pgvector real migration tests still skip cleanly without `PGVECTOR_TEST_DSN`. `docs/SOP_TRACK_M_P1_REAL_BENCHMARK_RUNS.md` and `docs/prompts/DEEPSEEK_TRACK_M_P1_REAL_BENCHMARK_RUNS_PROMPT.md` are the next operator handoff for real datasets, real judges, and BIL-2 publication evidence.
