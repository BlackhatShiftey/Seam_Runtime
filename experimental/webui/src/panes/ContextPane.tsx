...
import { fetchContext, type ContextResponse } from '../api/apiClient';
import Spinner from '../components/Spinner';

export default function ContextPane() {
  const [query, setQuery] = React.useState('');
  const [budget, setBudget] = React.useState(5);
  const [packBudget, setPackBudget] = React.useState(512);
  const [result, setResult] = React.useState<ContextResponse | null>(null);
  ...
  async function handleSubmit() {
    ...
    setResult(null);
    try {
      const res = await fetchContext({ query, budget, pack_budget: packBudget });
      setResult(res);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 16, overflowY: 'auto', flex: 1 }}>
      <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#4f8cfb', marginBottom: 12 }}>Context</div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Query for context pack..."
          style={{
            flex: 1,
            minWidth: 200,
            background: '#030d20',
        ...
        <input
          type="number"
          value={packBudget}
          onChange={(e) => setPackBudget(Number(e.target.value))}
          min={64}
          max={8192}
          title="Pack budget"
          style={{
        ...
        >
          {loading ? <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Spinner /><span>Packing...</span></span> : 'Pack'}
        </button>
      </div>
      ...
      {result && (
        <div>
          ...
          {result.candidates.length === 0 && (
            <div style={{ color: '#5a7a9a', fontSize: 12, marginBottom: 12 }}>No candidates</div>
          )}

          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#4f8cfb', marginBottom: 8, marginTop: 16 }}>Pack</div>
          <pre
            style={{
              background: '#020b1e',
